...
class News_model extends CI_Model
{
	...
	function check_cat_duplicates($slug)
    {
		// Loading second db and running query.
        $CI =& get_instance();
		$this->db2 = $CI->load->database('otherdb', TRUE);
		$query 	   = $this->db2->get_where('wp_terms', array(//making selection
			'slug' => $slug
		));
	...
	public function get_cat($slug)
	{
		...
		//setting the second parameter to TRUE (Boolean) the function will return the database object.
		$this->db2 = $CI->load->database('otherdb', TRUE);
		$this->db2->where('slug', $slug);
		//here we select every clolumn of the table
		$query = $this->db2->get('wp_terms');
		$data = $query->result_array();	
		return $data[0]['term_id'];
	}
	...
	public function insert_cat($data, $table_name)
    {
        /*$CI =& get_instance();
        $this->db2 = $CI->load->database('otherdb', TRUE);
